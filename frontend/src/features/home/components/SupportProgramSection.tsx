...
export interface SupportProgramSectionProps {
  ...
  onViewAll?: () => void
  /** 08 상품 상세로 이동. 넘기지 않거나 카드에 detailId가 없으면 버튼이 비활성됩니다. */
  ...
  return (
    <section className={cn('flex flex-col gap-6', className)}>
      ...
      <ul className="grid grid-cols-3 gap-6">
        {programs.map(({ id, detailId, name, status, limit, rate }) => (
          <li
            key={id}
          ...
          >
            ...
            <Button
              ...
              disabled={!onViewDetail || !detailId}
              onClick={onViewDetail && detailId ? () => onViewDetail(detailId) : undefined}
            >
              자세히 보기
            </Button>
          </li>
